...
#if defined(CONFIG_X86_64) && (LINUX_VERSION_CODE <= KERNEL_VERSION(5,10,0))
#define FTRACE_OPS_FL_RECURSION FTRACE_OPS_FL_RECURSION_SAFE
#define ARBITER_COMPATIBILITY_MODE 1
#endif
...
static int fh_resolve_hook_address(struct ftrace_hook *hook)
{
    ...
    if (!hook->address)
    {
        printk(KERN_DEBUG "iaudio: unresolved symbol: %s\n", hook->name);
        return -ENOENT;
    }
...
#ifdef ARBITER_COMPATIBILITY_MODE
...
int fh_install_hook(struct ftrace_hook *hook)
{
    ...
    if(err)
    {
        printk(KERN_DEBUG "iaudio: ftrace_set_filter_ip() failed: %d\n", err);
        return err;
    }
    ...
    if(err)
    {
        printk(KERN_DEBUG "iaudio: register_ftrace_function() failed: %d\n", err);
        return err;
    }
...
void fh_remove_hook(struct ftrace_hook *hook)
{
    ...
    if(err)
    {
        printk(KERN_DEBUG "iaudio: unregister_ftrace_function() failed: %d\n", err);
    }
